...
#Statistical summary for victim age, latitude, longitude, crime code
...
#cleans the data for the boxplot getting rid of - and X variables for Victim Sex and Victim Decent and H from Victim Sex
boxcrime_df = crime_df[(crime_df['Vict Descent'].isin(['-','X']) == False) & (crime_df['Vict Sex'].isin(['-','X','H']) == False)]


# In[50]:


#Box Plot of victim age vs sex
plt.figure(figsize=(8, 6))
sns.boxplot(x='Vict Sex', y='Vict Age', data=boxcrime_df)
...
#Boxplot of Victim Age vs Decent
plt.figure(figsize=(12, 8))
sns.boxplot(x='Vict Descent', y='Vict Age', data = boxcrime_df)
...
Explain this code and the reuslts of the code that shows in 2020 its in the 190000, 2021 is in the 200000, 2022 230000, 2023 230000, and 2024 is under 110000 but 2024 is not over
...
# Bar Graph of monthly counts
plt.figure(figsize=(14, 8))
sns.barplot(x=crimes_per_month_seasonal.index - 1, y=crimes_per_month_seasonal.values, palette='coolwarm')
plt.title('Crimes by Month (Seasonal Patterns)')
plt.xlabel('Month')
plt.ylabel('Number of Crimes')
plt.xticks(crimes_per_month_seasonal.index - 1, 
...
crime_df['Hour'] = (crime_df['TIME OCC'] // 100).astype(int)
crimes_by_hour = crime_df['Hour'].value_counts().sort_index()

# Bar Graph of based on crime counts for each hour of the day
plt.figure(figsize=(12, 6))
...
plt.figure(figsize=(12, 8))
plt.pie(status_counts.values, labels=[''] * len(status_counts), autopct=None, startangle=140, colors=sns.color_palette('pastel'))
plt.legend(status_counts.index, title='Crime Status', loc='upper right', bbox_to_anchor=(1.2, 1))
